...
export abstract class DiagramManagerImpl implements DiagramManager {
    ...
    open(uri: URI, input?: OpenerOptions): Promise<DiagramWidget> {
        const promiseDiagramWidget = this.getOrCreateDiagramWidget(uri, input)
    ...
    protected getOrCreateDiagramWidget(uri: URI, input?: OpenerOptions): Promise<DiagramWidget> {
        const widget = this.widgetRegistry.getWidget(uri, this.diagramType)
        if (widget !== undefined)
            return widget
        const newWidget = this.createDiagramWidget(uri, input)
        this.addToShell(newWidget)
        return Promise.resolve(newWidget)
    }

    protected createDiagramWidget(uri: URI, input?: OpenerOptions): DiagramWidget {
        ...
        newWidget.title.label = uri.path.base + "#" + input
